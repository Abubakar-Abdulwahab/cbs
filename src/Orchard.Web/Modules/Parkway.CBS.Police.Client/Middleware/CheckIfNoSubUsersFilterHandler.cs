...
{
    public class CheckIfNoSubUsersFilterHandler : FilterProvider, IActionFilter
    {
        ...
        public CheckIfNoSubUsersFilterHandler(Lazy<IHandler> handler, IAuthenticationService authenticationService, IOrchardServices orchardServices)
        {
                ...
                if (item.GetType() == typeof(HasSubUsersFilter))
                {
                    ...
                    try
                    {
                        if (!((HasSubUsersFilter)item).HasSubUsers())
                        {
                            ...
                            {
                                filterContext.Result = new RedirectToRouteResult(RouteName.SubUsersRequestList.ShowSubUsersRequestList, new RouteValueDictionary { });
                            }
                        }
                    }
                    catch (Exception exception)
                    { Logger.Error(exception, string.Format("Exception in CheckIfNoSubUsersFilterHandler. UserPartRecord Id {0}, Exception {1}", userPartId, exception.Message)); }
                    break;
                }
            }
        }
