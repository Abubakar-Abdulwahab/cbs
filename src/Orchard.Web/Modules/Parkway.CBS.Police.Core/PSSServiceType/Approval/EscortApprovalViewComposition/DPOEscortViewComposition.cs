...
{
    public class DPOEscortViewComposition : IEscortViewComposition
    {
        public int StageIdentifier => 1;
        private ITransactionManager _transactionManager;
        public ILogger Logger { get; set; }

        public DPOEscortViewComposition()
        {
        ...
        public bool DoValidation(EscortPartialVM item, EscortRequestDetailsVM objUserInput, ref List<ErrorModel> errors)
        {
            ...
            {
                //here we validate if the police officer exists and that the selected police officer belongs to the same command as the approving officer(DPO)
                officerLog = session.Query<PolicerOfficerLog>().Where(x => x.Id == officer.PoliceOfficerLogId).Select(x => new PoliceOfficerLogVM { Id = x.Id, IdentificationNumber = x.IdentificationNumber, CommandId = x.Command.Id, RankId = x.Rank.Id }).SingleOrDefault();
        ...
        /// </summary>
        public RequestApprovalResponse OnSubmit(int approverId, Int64 requestId, int commandTypeId)
        {
        ...
        private void SaveApprovalLog(EscortRequestDetailsVM objUserInput)
        {
            try
            {
                ...
                objUserInput.Comment = $"{adminUser.Fullname} (DPO {adminUser.Command.Name}): {objUserInput.Comment.TrimEnd('.')}. {additionalComment}";
        ...
        private decimal GetRateSheetId(long officerRankId, int pssEscortServiceCategoryId, int stateId, int lgaId, int commandTypeId)
        {
          ...
          && (er.State == new StateModel { Id = stateId }) && (er.LGA == new LGA { Id = lgaId }) && (er.CommandType == new CommandType { Id = commandTypeId }))
        ...
        /// <summary>
        /// Routes the request to DPO level
        /// </summary>
        /// <param name="item"></param>
        /// <param name="objUserInput"></param>
