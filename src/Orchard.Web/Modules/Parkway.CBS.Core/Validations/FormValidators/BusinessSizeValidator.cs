...
{
    public class BusinessSizeValidator : BaseValidator, IComplexFormValidator
    {
        ...
        public void ValidateFormFields(ref List<ErrorModel> errors, FormControlViewModel formField)
        {
            try
            {
                ...
                if (_transactionManager.GetSession().Query<PAYEBusinessSize>().Count(st => st.Size == formField.FormValue) != 1)
                {
                    errors.Add(new ErrorModel { ErrorMessage = ErrorLang.businesssize404().ToString(), FieldName = string.Format("[{0}].FormValue", formField.FormIndex) });
                }
            }
