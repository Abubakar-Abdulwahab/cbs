...
{
    public class StateDropDownValidator : BaseValidator, IComplexFormValidator
    {
        ...
        public void ValidateFormFields(ref List<ErrorModel> errors, FormControlViewModel formField)
        {
            try
            {
                ...
                if (_transactionManager.GetSession().Query<StateModel>().Count(st => st.Name == formField.FormValue) != 1)
                {
                    errors.Add(new ErrorModel { ErrorMessage = ErrorLang.statenotfound().ToString(), FieldName = string.Format("[{0}].FormValue", formField.FormIndex) });
                }
            }
