$(document).ready(function () {
        ...
        if (cursor == "moveright") {
            if (Page < PageSize) {
                ...
                if (requestsArray[Page] == undefined) {
                    var url = '/p/branch/get-next-request-list-page';
                    var requestData = { "operatorId": operatorId, "page": Page, "__RequestVerificationToken": token, "state": stateId, "lga": lgaId, "branch": selectedBranch, "status": reqStatus, "startDateFilter" : startDateFilter, "endDateFilter" : endDateFilter };
    ...
    function buildTableFromProcess(requestList, fromArr) {
        ...
        if (requestList.length >= 1) {
            ...
            for (var val in requestList) {
                ...
                var td5 = $('<td>' + requestList[val].BranchName + '</td>').appendTo(tr);

                var td6 = $('<td>' + requestList[val].ServiceName + '</td>').appendTo(tr);

                var td7 = $('<td>' + '<a href="' + getDetailsLink(requestList[val].FileRefNumber) + '" name = "ViewDetails"> View Details </a>' + '</td>').appendTo(tr);
    ...
    function getDetailsLink(fileRefNumber){
        return "/p/branch/request-details/" + fileRefNumber;
    }

    function getInvoiceLink(fileNumber){
        return "/p/request-invoice/" + fileNumber;
    }
