...
if (formationsAllocated != null) {
    ...
    formationsAllocated.forEach(function (value) {
        ...
        allocatedFormation.stateId = value.StateId;
        allocatedFormation.state = value.StateName;
...
function getFormation() {
    if (stateDropdown.options[stateDropdown.selectedIndex] == undefined || stateDropdown.options[stateDropdown.selectedIndex].value == "" || stateDropdown.options[stateDropdown.selectedIndex].value == "0") { alert("You need to select a state"); return; }
    ...
    requestedFormation.stateId = stateDropdown.options[stateDropdown.selectedIndex].text;
    requestedFormation.state = stateDropdown.options[stateDropdown.selectedIndex].text;
...
function buildTable() {
    ...
    formationsList.forEach(function (value, index) {
        ...
        let cell1 = row.insertCell(0).innerHTML = '' + value.state + '<input type="hidden" value=' + value.stateId + ' />';
        ...
        let cell6 = row.insertCell(5).innerHTML = "<span class='delete-formation-row' Title='Remove Item' onClick='removeFormation(" + index + ")'>Remove</span>";
    });
    clearInputs();
