$(document).ready(function () {
    ...
    $("#assignProviderFormSubmit").attr("disabled", false);
    $("#assignProviderFormCancel").attr("disabled", false);

    if (selectedMdas != null && selectedMdas.length > 0) { getRevenueHeadsPerMda(selectedMdas); }

    $("#assignProviderForm").submit(function (e) {
    ...
    function buildRevHeadDropDown(revHeads, mdaId, selector, checkAllRh = false) {
        ...
        if (allChecked) {
            listItems += "<li class = 'default-rev-head-" + mdaId + "'><label><input type='checkbox' class= 'default-rh' value=" + 0 + " name = 'SelectedRevenueHeads' id='mda" + mdaId + "' checked />  All</label></li>";
        } else {
            listItems += "<li class = 'default-rev-head-" + mdaId + "'><label><input type='checkbox' class= 'default-rh' value=" + 0 + " name = 'SelectedRevenueHeads' id='mda" + mdaId + "' />  All</label></li>";
        }
