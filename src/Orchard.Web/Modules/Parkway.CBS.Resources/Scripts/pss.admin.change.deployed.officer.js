...
function getOfficer() {
    ...
    if (totalOfficerPicked >= 1) { alert("You have already added an officer"); return; }
...
function buildTable() {
    let dpoOfficerTable = document.getElementById("relievingOfficerTable");
    officerList.forEach(function (value, index) {
        let row = dpoOfficerTable.insertRow(-1);
        let cell1 = row.insertCell(0).innerHTML = value.rank;
        let cell2 = row.insertCell(1).innerHTML = '' + value.name + ' <input type="hidden" name="selectedOfficer" value="' + value.officerLogId + '" />';
        let cell3 = row.insertCell(2).innerHTML = value.commandName;
        let cell4 = row.insertCell(3).innerHTML = value.id;
        let cell5 = row.insertCell(4).innerHTML = value.ippis;
        let cell6 = row.insertCell(5).innerHTML = value.accountNumber;
