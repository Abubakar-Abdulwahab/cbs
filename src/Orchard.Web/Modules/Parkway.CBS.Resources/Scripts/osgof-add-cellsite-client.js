        ...
        if (cursor == "moveright") {
            if (Page < PageSize) {
                ...
                if (cellSitesArray[Page] == undefined) {
                    var url = 'x/get-next-cellsites-schedule-page/' + scheduleRef + "/" + Page;
                    //var requestData = { "scheduleRef": scheduleRef, "page": Page, "__RequestVerificationToken": token };
    ...
    function buildTableFromProcess(cellSites, fromArr) {
        ...
        if (cellSites.length >= 1) {
            ...
            for (var val in cellSites) {
                ...
                if (cellSites[val].HasErrors) {
                    var td9 = $('<td>' + '<span class="status-false" data-toggle="tooltip" data-html = "true" data-placement="top" title="' + cellSites[val].ErrorMessages + '" >i</span>' + '</td>').appendTo(tr);
                }
                else {
                    var td9 = $('<td><span class="status-true" data-toggle="tooltip" data-placement="top">&#10003;</span></td>').appendTo(tr);
                }
            }
            $('[data-toggle="tooltip"]').tooltip();
