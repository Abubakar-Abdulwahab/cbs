...
var additionsArray = [];
var removalsArray = [];
var definitionLevelsRemovalsArray = [];
var definitionsRemovalsArray = [];
var serviceTypesRemovalsArray = [];
...
if (selectedUserType > 0) {
    ...
    if (removedCommands != null && removedCommands.length > 0) {
        $(removedCommands).each(function (index, value) {
            removalsArray.push(value);
        });
        collateRemovedCommands();
    }
    ...
    if (removedServiceTypes != null && removedServiceTypes.length > 0) {
        $(removedServiceTypes).each(function (index, selectedService) {
            serviceTypesRemovalsArray.push(selectedService);
        });
    }

    if (removedFlowDefinitions != null && removedFlowDefinitions.length > 0) {
        $(removedFlowDefinitions).each(function (index, selectedFlowDefinition) {
            definitionsRemovalsArray.push(selectedFlowDefinition);
        });
    }

    if (removedFlowDefinitionLevels != null && removedFlowDefinitionLevels.length > 0) {
        $(removedFlowDefinitionLevels).each(function (index, selectedFlowDefinitionLevel) {
            definitionLevelsRemovalsArray.push(selectedFlowDefinitionLevel);
        });
    }
    registerOnChangeEventOnFlowDefintitionLevelCheckboxList();
    collateRemovedArrayElements(definitionLevelsRemovalsArray, "removedFlowDefinitionLevels", "flowDefinitionLevelCheckboxList", "RemovedFlowDefinitionLevels[]");
    collateRemovedArrayElements(definitionsRemovalsArray, "removedFlowDefinitions", "flowDefinitionCheckboxList", "RemovedFlowDefinitions[]");
    collateRemovedArrayElements(serviceTypesRemovalsArray, "removedServiceTypes", "serviceCheckboxList", "RemovedServiceTypes[]");
}
...
$("#serviceCheckboxList input[type=checkbox]").change(function (e) {
    ...
    if (e.currentTarget.checked) {
        ...
        removeElementFromRemovalsArray(serviceTypesRemovalsArray, e.currentTarget.value);
        collateRemovedArrayElements(serviceTypesRemovalsArray, "removedServiceTypes", "serviceCheckboxList", "RemovedServiceTypes[]");
    } else {
        emptyDefinitionLevels($(".definition-" + e.currentTarget.value), e.currentTarget.value);
        $(".definition-" + e.currentTarget.value + " input[type=checkbox]:checked").each(function (index, checkboxInput) {
            addElementToRemovalsArray(definitionsRemovalsArray, selectedFlowDefintions, checkboxInput.value);
        });
        collateRemovedArrayElements(definitionLevelsRemovalsArray, "removedFlowDefinitionLevels", "flowDefinitionLevelCheckboxList", "RemovedFlowDefinitionLevels[]");
        collateRemovedArrayElements(definitionsRemovalsArray, "removedFlowDefinitions", "flowDefinitionCheckboxList", "RemovedFlowDefinitions[]");
        ...
        addElementToRemovalsArray(serviceTypesRemovalsArray, selectedServiceTypes, e.currentTarget.value);
        collateRemovedArrayElements(serviceTypesRemovalsArray, "removedServiceTypes", "serviceCheckboxList", "RemovedServiceTypes[]");
    }
});
...
function registerOnChangeEventOnCheckboxList(containerId, serviceTypeId) {
    $(containerId + " .multi-select-checkbox-options.definition-" + serviceTypeId + " input[type=checkbox]").change(function (e) {
        ...
        if (e.currentTarget.checked) {
            ...
            removeElementFromRemovalsArray(definitionsRemovalsArray, e.currentTarget.value);
            collateRemovedArrayElements(definitionsRemovalsArray, "removedFlowDefinitions", "flowDefinitionCheckboxList", "RemovedFlowDefinitions[]");
        } else {
            $(".parent-" + serviceTypeId + "-definition-level-" + e.currentTarget.value + " input[type=checkbox]:checked").each(function (index, checkboxInput) {
                addElementToRemovalsArray(definitionLevelsRemovalsArray, selectedFlowDefinitionLevels, checkboxInput.value);
            });
            collateRemovedArrayElements(definitionLevelsRemovalsArray, "removedFlowDefinitionLevels", "flowDefinitionLevelCheckboxList", "RemovedFlowDefinitionLevels[]");
            ...
            addElementToRemovalsArray(definitionsRemovalsArray, selectedFlowDefintions, e.currentTarget.value);
            collateRemovedArrayElements(definitionsRemovalsArray, "removedFlowDefinitions", "flowDefinitionCheckboxList", "RemovedFlowDefinitions[]");
        }
    });
}
...
function registerOnChangeEventOnFlowDefintitionLevelCheckboxList() {
    $("#flowDefinitionLevelCheckboxList .multi-select-checkbox-options input[type=checkbox]").change(function (e) {
        if (e.currentTarget.checked) {
            removeElementFromRemovalsArray(definitionLevelsRemovalsArray, e.currentTarget.value);
            collateRemovedArrayElements(definitionLevelsRemovalsArray, "removedFlowDefinitionLevels", "flowDefinitionLevelCheckboxList", "RemovedFlowDefinitionLevels[]");
        } else {
            addElementToRemovalsArray(definitionLevelsRemovalsArray, selectedFlowDefinitionLevels, e.currentTarget.value);
            collateRemovedArrayElements(definitionLevelsRemovalsArray, "removedFlowDefinitionLevels", "flowDefinitionLevelCheckboxList", "RemovedFlowDefinitionLevels[]");
        }
    })
}
...
function emptyDefinitionLevels(definitionCollection, serviceTypeId) {
    $(definitionCollection).each(function (index, val) {
        if (val.children[0] != undefined) {
            $(".parent-" + serviceTypeId + "-definition-level-" + val.children[0].children[0].value + " input[type=checkbox]:checked").each(function (index, checkboxInput) {
                addElementToRemovalsArray(definitionLevelsRemovalsArray, selectedFlowDefinitionLevels, checkboxInput.value);
            });
...
function removeCommand(index) {
    ...
    var removedItem = selectedCommandsList.splice(index, 1);
...
function addToAdditions(command) {
    if (originalSelectionArray.findIndex(function (val) { return val.Id == command.Id }) == -1) {
        additionsArray.push(command);
    }

    if (getIndexFromArray(removalsArray, command) > -1) {
        removalsArray.splice(getIndexFromArray(removalsArray, command), 1);
    }
}


function addToRemovals(command) {
    if (originalSelectionArray.findIndex(function (val) { return val.Id == command.Id }) > -1) {
        removalsArray.push(command);
    }

    if (getIndexFromArray(additionsArray, command) > -1) {
        additionsArray.splice(getIndexFromArray(additionsArray, command), 1);
    }
}


function removeElementFromRemovalsArray(targetArray, element) {
    if (getIndexOfArrayElement(targetArray, element) > -1) {
        targetArray.splice(getIndexOfArrayElement(targetArray, element), 1);
    }
}


function addElementToRemovalsArray(targetArray, originalArray, element) {
    if (getIndexOfArrayElement(originalArray, element) > -1) {
        targetArray.push(element);
    }
}


function getIndexFromArray(arr, command) {
    return arr.findIndex(function (val) { return val.Id == command.Id });
}


function getIndexOfArrayElement(arr, element) {
    return arr.findIndex(function (value) { return value == element });
}


function collateAddedCommands() {
    $(".addedCommands").remove();
    additionsArray.forEach(function (val, index) {
        $("#commandsTable").append('<input type="hidden" class="addedCommands" name="AddedCommands[' + index + '].Id" value=' + val.Id + ' /><input type="hidden" class="addedCommands" name="AddedCommands[' + index + '].Code" value="' + val.Code + '" /><input type="hidden" class="addedCommands" value="' + val.AccessType + '" name="AddedCommands[' + index + '].AccessType" />');
    });
}


function collateRemovedCommands() {
    $(".removedCommands").remove();
    removalsArray.forEach(function (val, index) {
        $("#commandsTable").append('<input type="hidden" class="removedCommands" name="RemovedCommands[' + index + '].Id" value=' + val.Id + ' /><input type="hidden" class="removedCommands" name="RemovedCommands[' + index + '].Code" value="' + val.Code + '" /><input type="hidden" class="removedCommands" value="' + val.AccessType + '" name="RemovedCommands[' + index + '].AccessType" />');
    });
}


function collateRemovedArrayElements(arr, elementClassname, elementContainerIdName, nameVal) {
    $("."+elementClassname).remove();
    arr.forEach(function (val) {
        $("#"+elementContainerIdName).append('<input type="hidden" class="' + elementClassname + '" name="' + nameVal + '" value=' + val + ' />');
    });
}
