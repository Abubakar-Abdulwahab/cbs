...
{
    public class DoPercentageSplitForNonFallFlag
    {
        ...
        public void DoSetSplitItemCount(string tenantName, long batchId)
        {
            ...
            try
            {
                if (!ProcessComp.CheckBatchStatus((Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status, Police.Core.Models.Enums.PSSSettlementBatchStatus.PerformPercentageSplitForNonFallFlag))
                {
                    UoW.BeginTransaction();
                    _batchDAOManager.UpdateSettlementBatchHasErrorAndErrorMessage(batchId, true, $"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.PerformPercentageSplitForNonFallFlag}");
                    UoW.Commit();
                    throw new Exception($"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.PerformPercentageSplitForNonFallFlag}");
                }
            }
            ...
            try
            {
                ...
                UoW.BeginTransaction();
                _feePartyCommandComputeManager.UpdateSplitItemPercentageForNonFallFlagValues(batchId);
                _batchDAOManager.UpdateBatchStatus(batchId, Police.Core.Models.Enums.PSSSettlementBatchStatus.PerformPercentageSplitForFallFlag, "Performing Percentage Split For Fall Flag");
                UoW.Commit();
                //once we are done we need to queue the next job
                StartHangFireService();
                BackgroundJob.Enqueue(() => new DoPercentageSplitForFallFlag().DoSetSplitItemCount(tenantName, batchId));
            }
            catch (Exception exception)
            {
                log.Error($"Error processing POSSAP settlement");
