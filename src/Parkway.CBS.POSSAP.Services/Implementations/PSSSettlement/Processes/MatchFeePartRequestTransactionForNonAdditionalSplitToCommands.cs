...
{
    public class MatchFeePartRequestTransactionForNonAdditionalSplitToCommands
    {
        ...
        public void DoMatchingOfFeePartiesTransactionsAndCommands(string tenantName, long batchId)
        {
            ...
            try
            {
                if (!ProcessComp.CheckBatchStatus((Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status, Police.Core.Models.Enums.PSSSettlementBatchStatus.MatchingFeePartiesWithRequestCommands))
                {
                    UoW.BeginTransaction();
                    _batchDAOManager.UpdateSettlementBatchHasErrorAndErrorMessage(batchId, true, $"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.MatchingFeePartiesWithRequestCommands}");
                    UoW.Commit();
                    throw new Exception($"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.MatchingFeePartiesWithRequestCommands}");
                }
            }
            ...
            try
            {
                ...
                UoW.BeginTransaction();
                _feePartyCommandComputeManager.ForNonAdditionalSplitsCombineFeePartyRequestTransactionWithCommands(batchId);
