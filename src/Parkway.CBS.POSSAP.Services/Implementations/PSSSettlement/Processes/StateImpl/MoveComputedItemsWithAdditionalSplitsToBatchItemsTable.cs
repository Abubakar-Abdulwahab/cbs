...
namespace Parkway.CBS.POSSAP.Services.Implementations.PSSSettlement.Processes.StateImpl
{
    public class MoveComputedItemsWithAdditionalSplitsToBatchItemsTable
    {
        ...
        public void MoveRecords(string tenantName, long batchId)
        {
            ...
            try
            {
                if (!ProcessComp.CheckBatchStatus((Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status, Police.Core.Models.Enums.PSSSettlementBatchStatus.MoveComputedItemsWithAdditionalSplitsForStateToBatchItemsTable))
                {
                    UoW.BeginTransaction();
                    _batchDAOManager.UpdateSettlementBatchHasErrorAndErrorMessage(batchId, true, $"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.MoveComputedItemsWithAdditionalSplitsForStateToBatchItemsTable}");
                    UoW.Commit();
                    throw new Exception($"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.MoveComputedItemsWithAdditionalSplitsForStateToBatchItemsTable}");
                }
            }
            ...
            try
            {
                ...
                UoW.BeginTransaction();
                _batchItemsManager.MoveAdditionalSplitRecordsForStateFromSplitComputeToBatchItems(batchId, "State");
                _batchDAOManager.UpdateBatchStatus(batchId, Police.Core.Models.Enums.PSSSettlementBatchStatus.MoveComputedItemsWithAdditionalSplitsForZonalToBatchItemsTable, "Moving Computed Items With Additional Splits And Zonal Adapter Value To Batch Items");
                UoW.Commit();
                //once we are done we need to queue the next job
                StartHangFireService();
                BackgroundJob.Enqueue(() => new ZonalImpl.MoveComputedItemsWithAdditionalSplitsToBatchItemsTable().MoveRecords(tenantName, batchId));
            }
            catch (Exception exception)
            {
                log.Error($"Error processing POSSAP settlement");
