...
{
    public class AddNonAdditionalSplitFeePartiesToBatchAggregate
    {
        ...
        public void AddAggregateRecords(string tenantName, long batchId)
        {
            ...
            try
            {
                if (!ProcessComp.CheckBatchStatus((Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status, Police.Core.Models.Enums.PSSSettlementBatchStatus.AddingNonAdditionalSplitFeePartiesToBatchAggregate))
                {
                    UoW.BeginTransaction();
                    _batchDAOManager.UpdateSettlementBatchHasErrorAndErrorMessage(batchId, true, $"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.AddingNonAdditionalSplitFeePartiesToBatchAggregate}");
                    UoW.Commit();
                    throw new Exception($"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.AddingNonAdditionalSplitFeePartiesToBatchAggregate}");
                }
            }
            ...
            try
            {
                ...
                UoW.BeginTransaction();
                _feePartyAggDAOManager.MoveToAggregateTableNonSplits(batchId);
                //_batchDAOManager.UpdateBatchStatus(batchId, Police.Core.Models.Enums.PSSSettlementBatchStatus.CheckingIfFeePartyInBatchHasAdditionalSplits, "Checking If Any Fee Parties In Batch Have Additional Splits");
                _batchDAOManager.UpdateBatchStatus(batchId, Police.Core.Models.Enums.PSSSettlementBatchStatus.AddingAdditionalSplitFeePartiesToPercentageRecalculationBatchAggregate, "Adding Fee Parties With Additional Splits To Percentage Recalculation Batch Aggregate");
                UoW.Commit();
                //once we are done we need to queue the next job
                StartHangFireService();
                //BackgroundJob.Enqueue(() => new CheckIfAnyFeePartyInBatchHasAdditionalSplits().CheckIfFeePartyInBatchHasAdditionalSplits(tenantName, batchId));
                BackgroundJob.Enqueue(() => new AddAdditionalSplitFeePartiesToPercentageRecalculationBatchAggregate().AddPercentageRecalculationAggregateRecords(tenantName, batchId));
            }
            catch (Exception exception)
            {
                log.Error($"Error processing POSSAP settlement");
