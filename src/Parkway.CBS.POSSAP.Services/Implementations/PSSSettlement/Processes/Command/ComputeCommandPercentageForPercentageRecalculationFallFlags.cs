...
{
    public class ComputeCommandPercentageForPercentageRecalculationFallFlags
    {
        ...
        {
            log.Info($"Computing command percentage for fall flag percentage recalculation fee party batch aggregate records for batch Id {batchId}");
            ...
            try
            {
                if (!ProcessComp.CheckBatchStatus((Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status, Police.Core.Models.Enums.PSSSettlementBatchStatus.ComputeCommandPercentageForPercentageRecalculationFallFlags))
                {
                    UoW.BeginTransaction();
                    _batchDAOManager.UpdateSettlementBatchHasErrorAndErrorMessage(batchId, true, $"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.ComputeCommandPercentageForPercentageRecalculationFallFlags}");
                    UoW.Commit();
                    throw new Exception($"Status mismatch for settlement batch with id {batchId}. Batch status - {(Police.Core.Models.Enums.PSSSettlementBatchStatus)batchDetails.Status} Expected status - {Police.Core.Models.Enums.PSSSettlementBatchStatus.ComputeCommandPercentageForPercentageRecalculationFallFlags}");
                }
            }
            ...
            try
            {
                ...
                UoW.BeginTransaction();
                _percentageRecalculationFeePartyBatchAggregate.ComputeCommandPercentageForFallFlags(batchId);
                _batchDAOManager.UpdateBatchStatus(batchId, Police.Core.Models.Enums.PSSSettlementBatchStatus.AddingAdditionalSplitFeePartiesToBatchAggregate, "Adding Additional Splits Fee Partie To Batch Aggregate");
                UoW.Commit();
                //once we are done we need to queue the next job
                StartHangFireService();
                BackgroundJob.Enqueue(() => new AddAdditionalSplitFeePartiesToBatchAggregate().AddAggregateRecords(tenantName, batchId));
            }
            catch (Exception exception)
            {
                log.Error($"Error processing POSSAP settlement");
