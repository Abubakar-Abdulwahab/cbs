...
namespace Parkway.CBS.POSSAP.Services.Implementations.PSSSettlement
{
    ...
    public class PSSSettlementServiceV2
    {
        ...
        //        Int64 recordCount = settlementRuleDAOManager.Count(x => ((x.IsActive) && (x.NextScheduleDate >= today) && (x.NextScheduleDate <= today.AddDays(1).AddMilliseconds(-1))));
        ...
        //        int skip = 0;
        //        StartHangfireServer();
        //        List<PSSSettlementRuleVM> activeSettlements = new List<PSSSettlementRuleVM>(chunkSize);
        //        while (stopper < pages)
        //        {
        //            activeSettlements = settlementRuleDAOManager.GetBatchActivePOSSAPSettlements(chunkSize, skip, today);
