...
{
    internal class DiscountJob : BaseJob, IJob
    {
        static int count = 0;
        public Task Execute(IJobExecutionContext context)
        {
            ...
            return Task.Run(() => {
                ...
                logger.Information(string.Format("Running discount job SN: {3} JN: {0} JG: {1} Date: {2}", jobDetail.Key.Name, jobDetail.Key.Group, DateTime.Now.ToLocalTime(), context.Scheduler.SchedulerName));
            });
        }
    }
}
